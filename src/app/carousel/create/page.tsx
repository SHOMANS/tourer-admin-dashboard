...
export default function CreateCarouselPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:3002'}/carousel`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${accessToken}`,
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Create Carousel Item</h1>
          <p className="mt-2 text-gray-600">Add a new carousel item for the mobile app home screen.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="flex justify-end space-x-3">
            ...
            >
              {loading ? 'Creating...' : uploadingImage ? 'Uploading...' : 'Create Carousel Item'}
            </button>
          </div>
        </form>
      </div>
